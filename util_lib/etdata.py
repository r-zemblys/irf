...
from util_lib.utils import round_up_to_odd, rolling_window
...
def get_px2deg(geom):
    ...
    px2deg = np.mean(
        (
            1
            / (
                np.degrees(2 * np.arctan(geom["screen_width"] / (2 * geom["eye_distance"])))
                / geom["display_width_pix"]
            ),
            1
            / (
                np.degrees(2 * np.arctan(geom["screen_height"] / (2 * geom["eye_distance"])))
...
def calc_event_data(
    etdata,
    evt,
    w={255: 1, 0: 1, 1: 50, 2: 1, 3: 1, 4: 1, 5: 1, 6: 1, "vel": 18, "etdq": 200},
):
    ...
    if e["e"] - e["s"] > w_etdq:
        ...
        rms = np.median(
            np.hypot(
                np.sqrt(np.mean(np.diff(x_) ** 2, axis=1)),
